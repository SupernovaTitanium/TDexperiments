...
THREADS="${THREADS:-36}"
T0="${T0:-0}"
OUT_ROOT="${OUT_ROOT:-td_cxx_logs_full_1e6_n36_11instances}"
...
MASTER_LOG="$OUT_ROOT/full_run_1e6_n36_11instances_${START_TS_HUMAN}.log"
...
echo "[info] rows_per_env=$ROWS_PER_ENV expected_total_rows=$EXPECTED_TOTAL_ROWS" | tee -a "$MASTER_LOG"
...
for env in "${ENVS[@]}"; do
  ...
  case "$env" in
    toyexample)
      # Force single-case run by fixing the default sweep key.
      EXTRA_ARGS+=(--set feature_omega_beta=1.0 --set scale_factor=1.0 --set seed=114514)
      ;;
    E1|E2)
      EXTRA_ARGS+=(--set eps2=1e-2 --set reward_mode=driven --set rho=1.0)
      ;;
    E3)
      EXTRA_ARGS+=(--set eps2=1e-2 --set reward_mode=signed --set rho=1.0)
      ;;
    E4)
      EXTRA_ARGS+=(--set feature_omega_beta=1.0 --set reward_mode=single-site --set rho=1.0)
      ;;
    E5)
      EXTRA_ARGS+=(--set feature_omega_beta=1.0 --set reward_mode=launch --set rho=1.0)
      ;;
    E6)
      EXTRA_ARGS+=(--set feature_omega_beta=1.0 --set reward_mode=single-harmonic --set rho=1.0)
      ;;
    E7)
      EXTRA_ARGS+=(--set feature_omega_beta=1.0 --set reward_mode=uniform --set rho=1.0)
      ;;
    E8)
      EXTRA_ARGS+=(--set eps2=1e-2 --set reward_mode=signed-cycle --set rho=1.0)
      ;;
    E9)
      EXTRA_ARGS+=(--set eps2=1e-2 --set reward_mode=linear --set rho=1.0)
      ;;
    E10)
      EXTRA_ARGS+=(--set eps2=1e-2 --set reward_mode=cluster-opposite --set rho=1.0)
      ;;
    *)
  ...
  ENV_ROWS="$(read_manifest_rows "$MANIFEST_PATH")"
