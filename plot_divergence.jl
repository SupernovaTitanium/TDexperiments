...
function plot_divergence(outdir::AbstractString)
    ...
    for (key, flist) in groups
        ...
        outpng = joinpath(plot_dir, @sprintf("%s__final__x-%s__eig-%.3e%s.png", env_name, pkey, lam, om_sfx))
        ...
        if !isempty(sel)
            ...
            outpng2 = joinpath(plot_dir, @sprintf("%s__curves__obj-A=(1-gamma)D__x-t__param-%s__eig-%.3e%s.png", env_name, pkey, lam, om_sfx))
            ...
            outpngA2 = joinpath(plot_dir, @sprintf("%s__curves__obj-A=(1-gamma)D+gammaS__x-t__param-%s__eig-%.3e%s.png", env_name, pkey, lam, om_sfx))
...
function plot_big_final_grid(outdir::AbstractString)
    ...
    for (sweeptype, files) in pairs(groups)
        ...
        plt.tight_layout()
        outpng = joinpath(plot_dir, @sprintf("%s__finalgrid__rows-%s.png", env_name, sweeptype==:c ? "c" : "alpha"))
        _safe_savefig(plt, outpng)
        Base.invokelatest(plt.close)
...
function plot_compact_c_grid(outdir::AbstractString)
    ...
    plt.tight_layout()
    outpng = joinpath(plot_dir, @sprintf("%s__compact__rows-c.png", env_name))
    _safe_savefig(plt, outpng)
    Base.invokelatest(plt.close)
...
function plot_learning_curve_grid(outdir::AbstractString)
    ...
    plt.tight_layout()
    outpng = joinpath(plot_dir, @sprintf("%s_learning_curves_grid_D.png", env_name))
    _safe_savefig(plt, outpng)
    Base.invokelatest(plt.close)
    ...
    plt.tight_layout()
    outpngA = joinpath(plot_dir, @sprintf("%s_learning_curves_grid_A.png", env_name))
...
function plot_best_learning_curves_by_param(outdir::AbstractString; sweeptype::Symbol=:alpha)
    ...
    outpng = joinpath(plot_dir, @sprintf("%s__bestcurves__by-%s.png", env_name, title_s))
    _safe_savefig(plt, outpng)
    Base.invokelatest(plt.close)
